...
const PhotographerDashboard: React.FC<PhotographerDashboardProps> = ({ gallery, onUpload, onBack }) => {
    ...
    <div className="w-full max-w-6xl mx-auto p-4 space-y-8 animate-fade-in">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-gray-800 pb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-400 text-sm">Manage your event gallery ({gallery.length} photos)</p>
          </div>
        </div>
      ...
      {gallery.length === 0 ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {gallery.map((photo) => (
            <div key={photo.id} className="relative aspect-square group overflow-hidden rounded-xl bg-gray-900">
              <img
                src={photo.url}
                alt="Gallery item"
